...
class LLMToolCallsHandler:
    ...
    def parse_tool(self, tool: type[LLMTool] | LLMDynamicTool, strict: bool = False):
        ...
        match self.client.llm_provider:
            case LLMProvider.OPENAI | LLMProvider.OLLAMA | LLMProvider.CUSTOM | LLMProvider.CODEX | LLMProvider.AZURE_OPENAI:
            ...
            case _:
                raise ValueError(
                    f"LLM provider must be one of: openai, anthropic, google, codex, azure_openai"
                )
    ...
    async def search_web_tool_call_handler(self, arguments: str) -> str:
        match self.client.llm_provider:
            case LLMProvider.OPENAI | LLMProvider.AZURE_OPENAI:
